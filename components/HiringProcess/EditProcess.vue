  ...
  methods: {
    open() {
      ...
      this.form = {
        ...
        rounds: this.process.hiring_process_round,
      }
    },
    close() {
      this.isOpen = false;
    ...
    addRound() {
      this.form.rounds.push({ name: '', process_id: this.process.id });
    },
    deleteRound(index) {
    ...
    async addHiringProcess() {
      this.$refs.form.validate(async (valid) => {
        ...
        const payloads = {
          name: this.form.name,
          hiring_process_round: this.form.rounds.map((round) => {
            return {
              name: round.name,
        ...
        try {
          const response = await HiringProcessApi.updateProcess(this.process.id ,payloads);
          this.close();
